...
function uniqueGuestSlug(orderId) {
  const stmt = db.prepare('SELECT 1 FROM guests WHERE order_id = ? AND slug = ?');
  let slug;
  do {
    slug = randomCode(5);
  } while (stmt.get(orderId, slug));
  return slug;
}

module.exports = { randomCode, uniqueOrderSlug, uniqueGuestSlug };
